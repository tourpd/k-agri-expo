...
import {
  ...
  listProblemContents,
  toggleProblemContentActive,
} from "@/lib/admin/problem-contents";

export const dynamic = "force-dynamic";

export default async function AdminProblemContentsPage() {
  const rows = await listProblemContents();

  async function createAction(formData: FormData) {
    "use server";
    await createProblemContent(formData);
    revalidatePath("/admin/problem-contents");
    revalidatePath("/expo");
  }
  ...
  async function toggleAction(formData: FormData) {
    ...
    await toggleProblemContentActive(id, nextValue);
    revalidatePath("/admin/problem-contents");
    revalidatePath("/expo");
  }
      ...
      <div style={S.container}>
        <div style={S.header}>
          <div>
            ...
            <h1 style={S.title}>농민 고민 콘텐츠 관리</h1>
            <p style={S.desc}>
              월별·시즌별로 메인에 노출할 농민 고민 콘텐츠를 등록합니다.
            </p>
          </div>
        </div>

        <section style={S.card}>
          <h2 style={S.sectionTitle}>새 콘텐츠 등록</h2>

          <form action={createAction} style={S.formGrid}>
            <label style={S.field}>
              <span style={S.label}>제목 *</span>
              <input name="title" style={S.input} placeholder="예: 3월에 심으면 망하는 작물 5가지" required />
            </label>

            <label style={S.field}>
              <span style={S.label}>부제</span>
              <input name="subtitle" style={S.input} placeholder="예: 봄 작형 시작 전 꼭 확인" />
            </label>
            ...
            <label style={{ ...S.field, gridColumn: "1 / -1" }}>
              ...
              <input name="link_url" style={S.input} defaultValue="/problems" required />
            </label>
        ...
        <section style={S.card}>
          <h2 style={S.sectionTitle}>등록된 콘텐츠</h2>
          ...
          <div style={S.tableWrap}>
            <table style={S.table}>
              ...
              <tbody>
                {rows.length === 0 ? (
                  <tr>
                    <td colSpan={8} style={S.emptyTd}>
                      아직 등록된 농민 고민 콘텐츠가 없습니다.
                    </td>
                  </tr>
                ) : (
                  rows.map((row) => (
                    <tr key={row.id}>
                      <td style={S.tdStrong}>{row.title}</td>

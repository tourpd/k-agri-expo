  ...
  async function startAnalysis() {
    ...
    if (!catalogFile) {
      setJob({
        ...
        totalChunks: 0,
        progress: 20,
        message: "회사 홈페이지·유튜브·블로그·제품자료를 읽고 고객마트용 제품을 분석중입니다.",
        result: null,
        error: null,
        logs: [
      ...
      if (!data.ok) {
        setJob({
          ...
          totalChunks: 0,
          progress: 100,
          message: "AI 고객마트 분석 실패",
          result: null,
          error: data.error ?? "AI 고객마트 분석 실패",
          logs: [
            `${new Date().toLocaleTimeString()} 오류: ${data.error ?? "AI 고객마트 분석 실패"}`,
          ],
          partialProducts: [],
        });
      ...
      setJob({
        ...
        totalChunks: 1,
        progress: 100,
        message: `AI 고객마트 분석 완료 / 발견 제품 ${found.length}개`,
        result: {
          fileName: "URL 자료 분석",
        ...
        error: null,
        logs: [
          `${new Date().toLocaleTimeString()} AI 고객마트 분석 완료 / 발견 제품 ${found.length}개`,
          `${new Date().toLocaleTimeString()} 제품 카드·고객마트 추천 구성 생성 완료`,
        ],
          ...
          <section className="rounded-3xl bg-white p-8 shadow-xl">
            ...
            {!activeProduct ? (
              ...
              <div className="mt-6 space-y-5">
                <PreviewBlock title="1. 문제 제기" text={`${activeProduct.name}이 필요한 고객 문제를 설명합니다.`} />

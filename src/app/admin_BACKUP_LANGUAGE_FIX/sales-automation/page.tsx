...
const consultingSteps: { key: StepKey; no: string; title: string; desc: string }[] = [
  ...
  { key: "meeting", no: "5", title: "AI 회의", desc: "전략·비평·고객심리" },
    ...
    try {
      ...
      const res = await fetch("/api/ai/create-farmer-mart", {
        ...
        body: JSON.stringify({
          ...
          blogUrl,
          memo:
            "기업진단 단계입니다. 업체의 실제 제품, 경쟁력, 부족한 근거, 고객/소비자 판매 가능성을 분석하세요. 근거 없는 제품을 지어내지 말고 확인 필요한 항목은 evidenceNeeded에 넣으세요.",
        }),
      });
        ...
        <section className="mt-8 rounded-3xl border-4 border-green-700 bg-green-50 p-6 text-black shadow-xl">
          ...
          {analysis && (
            <div className="mt-6 space-y-6">
              ...
              <div className="rounded-3xl bg-white p-6 shadow">
                <h3 className="text-3xl font-black text-black">고객/소비자 문제</h3>

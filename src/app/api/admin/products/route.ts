...
export async function GET(req: Request) {
  try {
    ...
    if (keyword) {
      query = query.or(
        `name.ilike.%${keyword}%,description.ilike.%${keyword}%,booth_id.ilike.%${keyword}%,campaign_tag.ilike.%${keyword}%`
      );
    }
    ...
    return NextResponse.json(
      {
        success: false,
        error: e?.message || "제품 목록 조회 중 오류가 발생했습니다.",
      },
      { status: 500 }
    );
  }
...
export async function POST(req: Request) {
  try {
    ...
    if (!name?.trim()) {
      return NextResponse.json(
        { success: false, error: "제품명은 필수입니다." },
        { status: 400 }
      );
    }
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        product_url: toNullableTrimmedString(product_url),
    ...
    return NextResponse.json(
      {
        success: false,
        error: e?.message || "제품 생성 중 오류가 발생했습니다.",
      },
      { status: 500 }
    );
  }

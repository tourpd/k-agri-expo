...
export async function POST(req: Request) {
  try {
    ...
    const estimatedValue = body.unit === "톤" ? qty * 1000 * price : qty * price;

    const { error } = await supabase.from("agri_assets").insert({
      asset_name: String(body.asset_name || `${body.producer_region || ""} ${body.product_name || ""}`).trim(),
    ...
    return NextResponse.json(
      { ok: false, error: error instanceof Error ? error.message : "등록 실패" },
      { status: 500 }
    );
  }
}

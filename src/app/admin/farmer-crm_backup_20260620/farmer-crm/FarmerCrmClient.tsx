    ...
    <main style={S.page}>
      <section style={S.header}>
        <div>
          ...
          <h1 style={S.title}>고객 CRM</h1>
          <p style={S.desc}>
            주문 데이터를 전화번호 기준으로 묶어 고객별 재구매 관리 화면으로 정리합니다.
          </p>
        </div>
      ...
      <section style={S.stats}>
        <Stat label="고객 수" value={totalFarmers} />
      ...
      <section style={S.filterCard}>
        ...
        <input
          ...
          placeholder="고객명 / 전화번호 / 지역 / 작물 / 등급 검색"
          style={S.search}
        />
            ...
            <thead>
              <tr>
                ...
                <th style={S.sortTh} onClick={() => toggleSort("repurchaseScore")}>
                  재구매점수 {sortLabel("repurchaseScore", sortField, sortDir)}
                </th>
                <th style={S.th}>VIP등급</th>
                <th style={S.th}>고객명</th>
                <th style={S.th}>전화번호</th>
                <th style={S.th}>지역</th>
                <th style={S.th}>작물</th>
                <th style={S.sortTh} onClick={() => toggleSort("orderCount")}>
                  주문수 {sortLabel("orderCount", sortField, sortDir)}
                </th>
                <th style={S.th}>총구매액</th>
                <th style={S.th}>포토닥터</th>
                <th style={S.sortTh} onClick={() => toggleSort("latestDate")}>
                  최근활동 {sortLabel("latestDate", sortField, sortDir)}
                </th>
                <th style={S.th}>관리</th>
              </tr>
            </thead>
            ...
            <tbody>
              {filteredRows.length === 0 ? (
                ...
                filteredRows.map((r) => {
                  ...
                  const grade = safe(r.latestStatus) || "일반";
                  const amountText = safe(r.latestProduct) || "-";
                    ...
                    <tr key={r.key}>
                      ...
                      <td style={S.tdStrong}>
                        <span
                          style={{
                            ...S.stageBadge,
                            color: stageColor(grade),
                            background: `${stageColor(grade)}18`,
                          }}
                        >
                          {grade}
                        </span>
                      ...
                      <td style={S.productTd}>{amountText}</td>
                      <td style={S.td}>{r.totalQuantity}회</td>

...
export default async function Page({
  ...
  const broadcast = ranked.slice(0, 10).map((p, i) => ({
    ...
    hook: `${p.crop || "고객"} 농가가 지금 놓치면 손해 보는 ${p.topic || p.disease_name || "핵심 포인트"}`,
    page: p.page_number,
  ...
  const shorts = ranked.slice(0, 10).map((p, i) => ({
    ...
    hook: `${p.crop || "고객"} 농사, 이것 하나 놓치면 수확량이 달라집니다`,
    page: p.page_number,
      ...
      <section className="p-6">
              ...
              <div className="rounded-xl border bg-slate-50 p-5">
                {mode === "broadcast" ? (
                  <>
                    ...
                    <p className="mt-3 text-3xl font-black">{selectedTitle} - 고객이 지금 놓치면 손해 보는 이유</p>
                    <h3 className="mt-6 text-2xl font-black">오프닝</h3>
                    <p className="mt-3 text-xl font-bold leading-8">오늘은 {selectedTitle}에 대해 이야기하겠습니다. 안이영 농업자료의 실제 페이지를 근거로 고객이 바로 실행할 내용을 정리합니다.</p>
                    <h3 className="mt-6 text-2xl font-black">구성</h3>
                    <ol className="mt-3 space-y-3">
                      {["문제 상황을 먼저 보여준다", "원본 자료 근거를 보여준다", "고객이 바로 할 행동을 제시한다", "관련 상품·공동구매로 자연스럽게 연결한다"].map((v, i) => (
                  ...
                  <>
                    ...
                    <ol className="mt-3 space-y-3">
                      {["왜 지금 중요한지 말한다", "고객이 확인할 증상을 보여준다", "오늘 할 행동 하나만 강하게 제시한다"].map((v, i) => (
                        <li key={v} className="rounded-lg bg-white p-4 text-lg font-black">{i + 1}. {v}</li>
                      ))}
                    </ol>
        ...
        <section className="mt-5 rounded-2xl border-4 border-emerald-600 bg-white p-6">
          <div className="grid grid-cols-[1.2fr_0.8fr] gap-5">
            <div className="rounded-2xl bg-emerald-50 p-6">
              ...
              <p className="mt-3 text-xl font-bold text-slate-700">이 자료는 방송소재, 쇼츠소재, 고객 행동지시, 상품연결까지 바로 활용 가능한 지식자산입니다.</p>
            </div>
        ...
        <div className="mt-5 grid grid-cols-2 gap-5">
          <section className="rounded-2xl border bg-white p-6">
            <h3 className="text-3xl font-black">이번주 고객 행동지시</h3>

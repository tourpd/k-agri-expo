      ...
      <section className="w-full px-8 py-6">
        <div className="rounded-xl border bg-white p-6">
          ...
          <p className="mt-2 text-lg font-bold text-slate-600">
            자료를 보는 화면이 아니라, 고객에게 보낼 캘린더·쇼츠·방송소재·농자재 연결 후보를 바로 보는 화면입니다.
          </p>
        ...
        <Block title="1. 고객 발송용 작물별 월별 캘린더 후보">
          <Table headers={["월", "작물", "고객에게 보낼 내용", "2주 전 준비 농자재", "실행"]}>
          ...
          <Table headers={["페이지", "작물", "방송 아이템", "현장 질문", "실행"]}>
            {broadcast.map((r) => (
              <tr key={r.id}>
                <td className="border px-3 py-3 text-center font-black">P{r.page_number}</td>
                <td className="border px-3 py-3"><Pill color="green">{r.crop || "공통"}</Pill></td>
                <td className="border px-4 py-3 text-lg font-black">{r.crop || "고객"} 농가가 꼭 알아야 할 {r.topic || "재배관리"}</td>

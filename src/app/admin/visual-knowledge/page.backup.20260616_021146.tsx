"use client";

import { useEffect, useMemo, useState } from "react";

type Row = {
  id: string;
  source_title?: string | null;
  source_name?: string | null;
  page_number?: number | null;
  image_url?: string | null;
  crop_name?: string | null;
  disease_name?: string | null;
  growth_stage?: string | null;
  key_info?: string | null;
  ai_summary?: string | null;
  action_instruction?: string | null;
  broadcast_material?: string | null;
  shorts_material?: string | null;
  pd_memo?: string | null;
  edit_request?: string | null;
  next_action?: string | null;
  db_location?: string | null;
  usage_flow?: string | null;
  created_at?: string | null;
};

const FLOW =
  "자료화면 → knowledge_visual_pages DB → AI 시각분석 → 판단규칙 DB → 방송소재 → 쇼츠 → 농민상담 → 행동지시";

export default function Page() {
  const [rows, setRows] = useState<Row[]>([]);
  const [selectedIds, setSelectedIds] = useState<string[]>([]);
  const [active, setActive] = useState<Row | null>(null);
  const [q, setQ] = useState("");
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);

  async function load() {
    setLoading(true);
    const res = await fetch("/api/admin/visual-knowledge", { cache: "no-store" });
    const json = await res.json();
    setRows(json.items || []);
    setLoading(false);
  }

  useEffect(() => {
    load();
  }, []);

  const filtered = useMemo(() => {
    const k = q.trim().toLowerCase();
    if (!k) return rows;
    return rows.filter((r) =>
      [
        r.source_title,
        r.source_name,
        r.crop_name,
        r.disease_name,
        r.growth_stage,
        r.key_info,
        r.ai_summary,
        r.next_action,
      ]
        .filter(Boolean)
        .join(" ")
        .toLowerCase()
        .includes(k)
    );
  }, [rows, q]);

  const stats = useMemo(() => {
    const total = rows.length;
    const analyzed = rows.filter((r) => r.crop_name && r.crop_name !== "분석필요").length;
    return {
      total,
      wait: total - analyzed,
      analyzed,
      selected: selectedIds.length,
    };
  }, [rows, selectedIds]);

  function toggle(id: string) {
    setSelectedIds((prev) =>
      prev.includes(id) ? prev.filter((v) => v !== id) : [...prev, id]
    );
  }

  function updateActive(key: keyof Row, value: string) {
    setActive((prev) => (prev ? { ...prev, [key]: value } : prev));
  }

  async function saveActive() {
    if (!active) return;
    setSaving(true);
    const res = await fetch("/api/admin/visual-knowledge", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(active),
    });
    if (!res.ok) {
      alert("저장 실패");
      setSaving(false);
      return;
    }
    await load();
    setSaving(false);
    alert("저장 완료");
  }

  async function analyzeOne(id?: string) {
    const targetId = id || active?.id || selectedIds[0];

    if (!targetId) {
      alert("분석할 자료를 선택하세요.");
      return;
    }

    setSaving(true);

    const res = await fetch("/api/admin/visual-knowledge/analyze", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id: targetId }),
    });

    const json = await res.json();

    if (!res.ok) {
      alert(json.error || "AI 분석 실패");
      setSaving(false);
      return;
    }

    if (active?.id === targetId && json.item) {
      setActive({ ...active, ...json.item });
    }

    await load();
    setSaving(false);
    alert("GPT Vision 분석 완료");
  }

  return (
    <main className="min-h-screen bg-[#f4f7f2] p-4 text-black">
      <section className="mb-3 flex items-start justify-between gap-3">
        <div>
          <div className="text-xs font-black text-green-700">
            K-AGRI EXPO / 농업 AI 두뇌센터
          </div>
          <h1 className="text-3xl font-black tracking-tight">
            자료화면 AI DB센터
          </h1>
          <p className="mt-1 text-sm text-gray-600">
            PDF·PPT·이미지·유튜브 캡처·토양검정서까지 추가 업로드를 계속 받아 AI 두뇌로 축적하는 엑셀형 운영표입니다.
          </p>
        </div>

        <div className="flex gap-2">
          <button onClick={load} className="rounded-xl bg-green-700 px-4 py-3 text-sm font-black text-white">
            DB 새로고침
          </button>
          <button className="rounded-xl bg-black px-4 py-3 text-sm font-black text-white">
            + 새 자료 업로드
          </button>
          <button
            onClick={() => analyzeOne()}
            className="rounded-xl bg-gray-900 px-4 py-3 text-sm font-black text-white"
          >
            선택자료 AI분석
          </button>
        </div>
      </section>

      <section className="mb-3 grid grid-cols-4 gap-2">
        <Stat label="전체 자료" value={stats.total} />
        <Stat label="분석대기" value={stats.wait} />
        <Stat label="분석완료" value={stats.analyzed} />
        <Stat label="선택자료" value={stats.selected} />
      </section>

      <section className="mb-3 rounded-xl border border-green-200 bg-green-50 p-3 text-sm">
        <b>활용 흐름</b> : {FLOW}
      </section>

      <section className="mb-2 flex gap-2">
        <input
          value={q}
          onChange={(e) => setQ(e.target.value)}
          placeholder="자료명, 작물, 병해충, 핵심정보 검색"
          className="flex-1 rounded-xl border border-gray-300 bg-white px-4 py-3 text-sm"
        />
        <button className="rounded-xl border bg-white px-4 py-3 text-sm font-black">
          선택 {selectedIds.length}건
        </button>
      </section>

      <section className="rounded-2xl border bg-white shadow">
        <div className="max-h-[72vh] overflow-auto">
          <table className="min-w-[1180px] w-full border-collapse text-xs">
            <thead className="sticky top-0 z-10 bg-gray-950 text-white">
              <tr>
                <Th w="48px">선택</Th>
                <Th w="90px">이미지</Th>
                <Th w="240px">자료명</Th>
                <Th w="70px">페이지</Th>
                <Th w="120px">작물</Th>
                <Th w="140px">병해충</Th>
                <Th w="420px">AI요약</Th>
                <Th w="220px">다음작업</Th>
              </tr>
            </thead>

            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={8} className="p-8 text-center">불러오는 중...</td>
                </tr>
              ) : filtered.length === 0 ? (
                <tr>
                  <td colSpan={8} className="p-8 text-center">자료가 없습니다.</td>
                </tr>
              ) : (
                filtered.map((r) => (
                  <tr
                    key={r.id}
                    onClick={() =>
                      setActive({
                        ...r,
                        db_location: r.db_location || "knowledge_visual_pages",
                        usage_flow: r.usage_flow || FLOW,
                      })
                    }
                    className="cursor-pointer border-b hover:bg-green-50"
                  >
                    <Td center>
                      <input
                        type="checkbox"
                        checked={selectedIds.includes(r.id)}
                        onClick={(e) => e.stopPropagation()}
                        onChange={() => toggle(r.id)}
                      />
                    </Td>
                    <Td>
                      {r.image_url ? (
                        <img src={r.image_url} className="h-12 w-16 rounded border object-contain" alt="" />
                      ) : (
                        <div className="h-12 w-16 rounded border bg-gray-100 text-center text-[10px] leading-[48px]">
                          없음
                        </div>
                      )}
                    </Td>
                    <Td strong>{r.source_title || r.source_name || "자료명 필요"}</Td>
                    <Td center>{r.page_number || "-"}</Td>
                    <Td>{r.crop_name || "분석필요"}</Td>
                    <Td>{r.disease_name || "-"}</Td>
                    <Td>{r.ai_summary || "-"}</Td>
                    <Td>{r.next_action || "GPT Vision 분석"}</Td>
                  </tr>
                ))
              )}
            </tbody>
          </table>
        </div>
      </section>

      {active && (
        <section className="fixed inset-0 z-50 flex bg-black/20">
          <div className="ml-auto flex h-screen w-[680px] flex-col bg-white shadow-2xl">
            <div className="flex items-start justify-between border-b p-4">
              <div>
                <div className="text-xs font-black text-green-700">
                  세환PD × AI 공동편집실
                </div>
                <h2 className="text-xl font-black">
                  {active.source_title || active.source_name || "자료 상세"}
                </h2>
                <p className="text-xs text-gray-500">
                  페이지 {active.page_number || "-"} / {active.db_location || "knowledge_visual_pages"}
                </p>
              </div>
              <button
                onClick={() => setActive(null)}
                className="rounded-lg border px-3 py-2 text-sm font-black"
              >
                닫기
              </button>
            </div>

            <div className="flex-1 overflow-y-auto p-4">
              <div className="mb-3 rounded-xl border bg-gray-50 p-2">
                {active.image_url ? (
                  <img
                    src={active.image_url}
                    className="mx-auto max-h-[300px] w-full object-contain"
                    alt=""
                  />
                ) : (
                  <div className="flex h-48 items-center justify-center text-gray-500">
                    이미지 없음
                  </div>
                )}
              </div>

              <div className="mb-3 grid grid-cols-3 gap-2">
                <Field label="작물" value={active.crop_name || ""} onChange={(v) => updateActive("crop_name", v)} />
                <Field label="병해충" value={active.disease_name || ""} onChange={(v) => updateActive("disease_name", v)} />
                <Field label="생육단계" value={active.growth_stage || ""} onChange={(v) => updateActive("growth_stage", v)} />
              </div>

              <Field label="자료명" value={active.source_title || ""} onChange={(v) => updateActive("source_title", v)} />

              <Area small label="핵심정보" value={active.key_info || ""} onChange={(v) => updateActive("key_info", v)} />
              <Area small label="AI요약" value={active.ai_summary || ""} onChange={(v) => updateActive("ai_summary", v)} />
              <Area small label="방송소재" value={active.broadcast_material || ""} onChange={(v) => updateActive("broadcast_material", v)} />
              <Area small label="쇼츠소재" value={active.shorts_material || ""} onChange={(v) => updateActive("shorts_material", v)} />
              <Area small label="농민 행동지시" value={active.action_instruction || ""} onChange={(v) => updateActive("action_instruction", v)} />
              <Area small label="세환PD 메모" value={active.pd_memo || ""} onChange={(v) => updateActive("pd_memo", v)} />
              <Area small label="수정요청 / 다음작업" value={active.next_action || ""} onChange={(v) => updateActive("next_action", v)} />
            </div>

            <div className="border-t bg-white p-4">
              <button
                onClick={() => analyzeOne(active.id)}
                disabled={saving}
                className="mb-2 w-full rounded-xl bg-black py-3 text-base font-black text-white"
              >
                {saving ? "분석 중..." : "GPT Vision 분석"}
              </button>

              <button
                onClick={saveActive}
                disabled={saving}
                className="w-full rounded-xl bg-green-700 py-3 text-base font-black text-white"
              >
                {saving ? "저장 중..." : "저장"}
              </button>
            </div>
          </div>
        </section>
      )}
    </main>
  );
}

function Stat({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-xl border bg-white p-4">
      <div className="text-xs font-black text-gray-500">{label}</div>
      <div className="mt-1 text-2xl font-black">{value.toLocaleString()}</div>
    </div>
  );
}

function Th({ children, w }: { children: React.ReactNode; w: string }) {
  return (
    <th style={{ width: w }} className="border-r border-gray-800 px-3 py-3 text-left font-black">
      {children}
    </th>
  );
}

function Td({
  children,
  center,
  strong,
}: {
  children: React.ReactNode;
  center?: boolean;
  strong?: boolean;
}) {
  return (
    <td
      className={[
        "max-w-[360px] overflow-hidden text-ellipsis whitespace-nowrap border-r px-3 py-2 align-middle",
        center ? "text-center" : "",
        strong ? "font-black" : "",
      ].join(" ")}
    >
      {children}
    </td>
  );
}

function Badge({ children }: { children: React.ReactNode }) {
  return (
    <span className="rounded-full bg-yellow-100 px-2 py-1 text-[11px] font-black text-yellow-800">
      {children}
    </span>
  );
}

function Field({
  label,
  value,
  onChange,
}: {
  label: string;
  value: string;
  onChange: (v: string) => void;
}) {
  return (
    <label className="mb-3 block">
      <div className="mb-1 text-xs font-black text-gray-600">{label}</div>
      <input
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className="w-full rounded-lg border px-3 py-2 text-sm"
      />
    </label>
  );
}

function Area({
  label,
  value,
  onChange,
  small,
}: {
  label: string;
  value: string;
  onChange: (v: string) => void;
  small?: boolean;
}) {
  return (
    <label className="mb-2 block">
      <div className="mb-1 text-xs font-black text-gray-600">{label}</div>
      <textarea
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className={[
          "w-full rounded-lg border px-3 py-2 text-sm leading-relaxed",
          small ? "min-h-16" : "min-h-24",
        ].join(" ")}
      />
    </label>
  );
}

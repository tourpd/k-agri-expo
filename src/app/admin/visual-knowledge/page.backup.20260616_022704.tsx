...
const FLOW =
  "자료 업로드 → 이미지 DB → GPT Vision 자동분석 → 검수 → 판단규칙 DB → 방송소재 → 쇼츠 → 농민상담 → 행동지시";

function getVisualType(r: Row) {
  return r.raw?.ai_visual_type || r.raw?.visual_type || "자료화면";
}

function getStatus(r: Row) {
  const status = r.raw?.ai_status;
  if (status === "vision_complete") return "분석완료";
  if (r.crop_name && r.crop_name !== "분석필요") return "검수필요";
  return "분석대기";
}

function getReading(r: Row) {
...
export default function Page() {
  ...
  const stats = useMemo(() => {
    ...
    const done = rows.filter((r) => getStatus(r) !== "분석대기").length;
    const wait = total - done;
    const rules = rows.filter((r) => String(r.next_action || "").includes("판단규칙")).length;
    return { total, wait, done, rules, selected: selectedIds.length };
  }, [rows, selectedIds]);
  ...
  const filtered = useMemo(() => {
    ...
    if (filter !== "전체") {
      list = list.filter((r) => {
        if (filter === "분석대기") return getStatus(r) === "분석대기";
        if (filter === "분석완료") return getStatus(r) !== "분석대기";
  ...
  async function analyzeOne(id?: string) {
    ...
    if (active?.id === targetId && json.item) setActive({ ...active, ...json.item });
  ...
  async function analyzeAll() {
    const targets = selectedIds.length
      ...
      : rows.filter((r) => getStatus(r) === "분석대기");
    ...
    <main className="min-h-screen bg-[#f4f7f2] p-4 text-black">
      <section className="mb-3">
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="text-xs font-black text-green-700">K-AGRI EXPO / 농업 AI 두뇌센터</div>
            <h1 className="text-3xl font-black tracking-tight">자료화면 AI DB센터</h1>
        ...
        <div className="mt-3 grid grid-cols-5 gap-2">
          ...
          <Stat label="판단규칙 후보" value={stats.rules} />
                ...
                {loading ? (
                  ...
                  filtered.map((r) => (
                    ...
                    >
                      ...
                      <Td>{getVisualType(r)}</Td>
                      <Td><Badge status={getStatus(r)} /></Td>
                      <Td>{getReading(r)}</Td>
